...
def connectivity_matrix(mol):
    """
    Setup the connectivity matrix for an rdkit-mol object based
    on neighbor lists.

    input: rdkit molecule object
    return: n_atom * n_atom connectivity matrix
    """
    atoms = mol.GetAtoms()
...
def num_carbons(mol):
    """
    Wrapper to return the number of carbon atoms in a rdkit molecule

    input: rdkit molecule
    return: number of carbon atoms (integer)
    """
    atoms = mol.GetAtoms()
...
def is_fully_aromatic(mol):
    """
    Determine whether the rdkit molecule is fully aromatic
...
def choose_n(myid, choices, n_molecules=10):
    """
    Choose a number of random molecules from a given list based
    on a random seed

    input: random seed (student number, integer)
           choices (array of possible molecules, list)
           n_molecules (number of required samples, integer)
...
def draw_MO(mol, eigenvecs=None, n=0):
    """
